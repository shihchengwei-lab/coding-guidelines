...
- `rules.sh` — POSIX shell script（Linux / macOS / WSL / Git Bash 用）
- `rules.py` — Python alternative（Windows native 推薦；已 force UTF-8 stdout）
- `settings.example.json` — 範例配置（`UserPromptSubmit` event，Linux/macOS 路徑風格；Windows 看下面第 2 節最後的替換）
...
規則寫在 `rules.sh` / `rules.py` 內。直接編輯字串就行。

> Windows 使用者注意：`rules.py` 開頭兩行 `import sys; sys.stdout.reconfigure(encoding="utf-8")` 是給 Windows native Python 用的——預設 stdout 用系統編碼（如 cp950），中文會輸出成亂碼。改寫時保留這兩行。Linux/macOS 不需要。
